...
import histoycompany from "../models/histoycompany";
import { convertResults, getConvertedValue } from "../utils/utils";

@odata.type(OptionSet)
class histoycompanyController extends ODataController {
  /**
   * Get All OptionSet
   */
  @odata.GET
  async find() {
    let result = await global.connection
      .request()
      .query("select h.check_date,e.employee_id,e.first_name,e.last_name,h.result from m_employee e left outer join m_company c on (e.company_id=c.company_id) left outer join t_atk_history h on (e.employee_id=h.employee_id) ORDER BY check_date DESC");
